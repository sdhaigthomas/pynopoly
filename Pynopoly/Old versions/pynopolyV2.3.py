...
def cards(card_type):
    ...
    #print("appending:", card, "to the end of community_chest")
    card_type.pop(0)
    #print("removing ", card, "from the top of the list")
...
def check_balance1(player_no, cost, prop_n, transaction_type):
    ...
    if balance >= cost:
        ...
        if transaction_type == "rent":
            pay_rent(player_no, cost, balance)
        else:
...
def pay_rent(player_no, cost, balance):
    rent = abs(cost)
    ...
    if squares[square_no][4] == 'bank':
        ...
        if squares[square_no][4] == "NA":
            if squares[square_no][1] == "GO":
                ...
                input("Income Tax!")
                pay_rent(player_no, -200, player[player_no][2])
...
def new_square_cal(player_no, die1, die2):
    ...
    if new_player_pos > 39:
        ...
        go(player_no) # When you go past GO! or when you land on GO! then collect £200
    else:
